...
export function NotificationsPage() {
  ...
  return <div className="page-wrap">
    ...
    {items.length ? <div className="panel compact-list">{items.map((item) => <div key={item.id}><strong><Bell size={16} /> {item.title}</strong><span>{item.message}</span><span>{new Date(item.created_at).toLocaleString('vi-VN')} · {item.read_at ? 'Đã đọc' : 'Chưa đọc'}</span>{!item.read_at && <button className="button secondary" onClick={() => void markRead(item.id)}>Đánh dấu đã đọc</button>}</div>)}</div> : <EmptyState title="Chưa có thông báo" description="Thông báo giao từ sẽ xuất hiện ở đây." />}
  </div>;
}
