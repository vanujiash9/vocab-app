...
export function FlashcardsPage() {
  ...
  return <div className="page-wrap"><div className="page-heading"><div><span>Review session</span><h1>Flashcard ôn tập</h1><p>Nhấn vào thẻ để lật mặt và tự đánh giá mức độ ghi nhớ.</p></div></div><section className="flash-layout"><button className={`flash-card ${flipped ? 'flipped' : ''}`} onClick={() => setFlipped(!flipped)}><div className="flash-front panel"><span>Từ {index + 1}/{items.length}</span><h2>{item.word}</h2><p>{item.phonetic || 'Nhấn để xem nghĩa'}</p></div><div className="flash-back panel"><span>Nghĩa</span><h3>{item.vietnamese_meaning}</h3><p>{item.example_sentence}</p></div></button><aside className="panel flash-side"><RotateCcw size={28} /><h3>Tự đánh giá</h3><p>Đánh dấu từ để hệ thống ưu tiên ôn tập phù hợp.</p><button className="button secondary full" onClick={() => void mark('difficult')}>Khó nhớ</button><button className="button primary full" onClick={() => void mark('known')}>Đã thuộc</button><button className="text-button" onClick={next}>Bỏ qua →</button></aside></section></div>;
}
