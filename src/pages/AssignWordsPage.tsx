...
function AssignHeading({ canAssign, actionLabel, onAssign, onImport }: { canAssign: boolean; actionLabel: string; onAssign: () => void; onImport: () => void }) {
  return <div className="page-heading assign-page-heading teacher-assign-heading"><div><span>Teacher assignment</span><h1>Giao từ</h1><p>Chọn học viên, chọn từ rồi xác nhận trong một flow ngắn gọn.</p></div><div className="assign-heading-actions teacher-assign-actions"><button className="button secondary" onClick={onImport}><FileSpreadsheet size={17} /> Nhập CSV</button><button className="button primary" disabled={!canAssign} onClick={onAssign}><Send size={17} /> {actionLabel}</button></div></div>;
}
...
export function AssignWordsPage() {
  ...
  return <div className="page-wrap assign-page-wrap teacher-assign-page"><AssignHeading canAssign={canSubmit} actionLabel={assignLabel} onAssign={() => void assign()} onImport={() => navigate('/import-excel')} />{message && <div className="form-message standalone">{message}</div>}{submitError && <div className="form-message standalone">{submitError}</div>}<section className="assign-hero-grid compact teacher-assign-grid"><div className="assign-grid compact teacher-assign-columns"><StudentPanel students={students} selectedStudents={selectedStudents} onToggle={toggleStudent} onNavigate={() => navigate('/students')} /><WordsPanel words={filteredWords} selectedWords={selectedWords} query={query} onQueryChange={setQuery} onToggle={toggleWord} onNavigate={() => navigate('/lookup')} /></div><AssignSummary selectedStudentsCount={selectedStudents.length} selectedWordsCount={selectedWords.length} canAssign={canSubmit} actionLabel={summaryAssignLabel} onAssign={() => void assign()} /></section></div>;
}
