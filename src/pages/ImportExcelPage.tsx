...
export function ImportExcelPage() {
  ...
  return <div className="page-wrap import-page-wrap"><div className="page-heading import-page-heading"><div><span>Teacher resource import</span><h1>Import Excel</h1><p>Ưu tiên CSV export từ Excel để nhập nhiều từ trong một bước gọn rồi quay lại giao bài.</p></div></div>{message && <div className="form-message standalone">{message}</div>}{error && <div className="form-message standalone import-error-message">{error}</div>}<section className="assign-hero-grid compact"><article className="panel import-dropzone"><div className="import-dropzone-head"><div className="icon-button import-dropzone-icon"><Upload size={20} /></div><div><h3>Chọn file CSV từ Excel</h3><p>Dùng file CSV UTF-8 để hệ thống đọc cột dữ liệu, tạo preview và lưu vào kho từ của teacher.</p></div></div><input ref={inputRef} className="import-file-input" type="file" accept=".csv,text/csv" onChange={(event) => void handleFileChange(event)} />{preview ? <ImportPreviewCard preview={preview} /> : <EmptyState title={isParsing ? 'Đang đọc file CSV' : 'Chưa có file được chọn'} description={isParsing ? 'Hệ thống đang đọc file, chuẩn hóa cột và tạo preview trước khi nhập.' : 'Chọn một file CSV export từ Excel để xem trước số dòng hợp lệ rồi mới nhập.'} primaryAction={{ label: isParsing ? 'Đang xử lý...' : 'Chọn file CSV', onClick: pickFile, variant: 'secondary' }} secondaryAction={{ label: 'Quay lại giao từ', onClick: () => navigate('/assign-words'), variant: 'secondary' }} />}<div className="assign-heading-actions import-actions">{preview && <button className="button secondary" onClick={pickFile} disabled={isParsing || isSubmitting}><FileSpreadsheet size={17} /> Chọn file khác</button>}<button className="button primary" onClick={() => void handleImport()} disabled={!canSubmit}><Upload size={17} /> {isSubmitting ? 'Đang import...' : 'Import vào kho từ'}</button></div><p className="import-hint">ponytail: pass này hỗ trợ CSV export từ Excel; thêm parser .xlsx thật khi cần đọc workbook native.</p></article><aside className="assign-summary-card compact import-summary-card"><div className="assign-summary-head compact"><div><span className="eyebrow assign-eyebrow">Tài nguyên</span><h3>Checklist trước khi nhập</h3><p>Giữ dữ liệu sạch để thư viện và đợt giao từ dễ quản lý hơn.</p></div><div className="icon-button import-dropzone-icon"><FileSpreadsheet size={20} /></div></div><ImportChecklist /></aside></section></div>;
}
